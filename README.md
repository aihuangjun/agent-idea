...
- **目录树**：IDEA 习惯（单击选中、双击打开 / 展开），按 git 状态着色（修改蓝、新增绿、删除灰、忽略灰）；右键「重命名…」或选中后按 ⇧F6 重命名（已跟踪的走 `git mv`，开着的标签跟着换）；右键「删除…」或选中后按 ⌫ 删文件 / 目录（进废纸篓，删前确认）；「定位当前文件」（⌥⌘L）；右键 `.sh` / `.zsh` / `.fish` 脚本「在终端中运行」。
