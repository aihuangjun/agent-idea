...
- **目录树 + 编辑**：代码在编辑器（CodeMirror）里按语言高亮、可直接修改，⌘S 保存，切到别的应用或关标签自动保存；Markdown 预览 / 源码 / 分栏三种看法（含 mermaid）；图片预览。超过 2MB 的文件只读。
- **后退 / 前进**（⌥← / ⌥→）：按打开顺序在标签间来回。
- **在终端中运行**：右键 `.sh` 脚本，在系统终端里跑。
- **Git**：显示当前分支，目录树按 IDEA 的配色标注修改 / 新增 / 删除 / 未跟踪 / 忽略。
- **变更、diff 与提交**：像 IDEA 提交窗口那样列出所有变更，点一条看 diff（并排 / 单列、词级高亮），工作区变更的 diff 可以直接改（并排时右边、单列时整个就是工作区文件）；勾选要提交的文件，填信息，「提交」或「提交并推送」；右键可回滚或删除。
- **提交历史**（⌘9）：当前分支的提交列表，选中看它改了哪些文件、每个文件的 diff。
- **查找文件**（⌘F）：按文件名 / 路径模糊搜索，回车打开并在树上定位。
