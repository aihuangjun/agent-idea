...
public struct FlattenedTree: Sendable {
    ...
    /// 一个节点改了名或挪了地方：它和它下面的目录展开状态跟着换路径；这些目录的子节点缓存作废
    /// （里面节点的 URL 还是旧的），新旧父目录都要重列。都由下一次 `rows(root:)` 前的 `needsLoading` 补加载。
    public mutating func rename(_ path: String, to newPath: String) {
        ...
        children[(newPath as NSString).deletingLastPathComponent] = nil
    }
