...
struct Project: Equatable {
    ...
    init(root: URL, repositoryRoot: URL? = nil) {
    ...
    func repositoryRelativePath(of url: URL) -> String? {
    ...
    /// 反过来：仓库相对路径 → 绝对 URL。
    func url(forRepositoryPath relative: String) -> URL? {
        repositoryRoot?.appendingPathComponent(relative)
    }

    /// 给面包屑用：相对项目根的路径段。
    func projectRelativeComponents(of url: URL) -> [String] {
