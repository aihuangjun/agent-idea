...
- **目录树的鼠标交互是 `Views/TreeRowInteraction`**（NSView 直包，套在每一行上）：按下即选中、松开结算点击（双击仍由 `DoubleClickDetector` 判）、拖过 4pt 起 `NSDraggingSession`（剪贴板类型 `local.agentidea.tree-node`，值是绝对路径，只在应用内有效；拖影是 `DragPreview` 画的「图标 + 名字」小块，不截整行），目录行 / 文件行 / 根行都是拖放目标（`TreeDropTarget`：文件行代表它所在的目录，高亮画在**接收的目录那一行**上，由树的 `dropDirectory` 状态算好传给行）。折叠目录上停 0.6s 自动展开（`springLoad`），靠近列表上下边缘自动滚动（`enclosingScrollView`，SwiftUI 的 ScrollView 在 macOS 上是 NSScrollView）。**这两个定时器必须挂在 `.common` 模式**（`View.schedule`）：拖拽期间 AppKit 的事件循环在 eventTracking 模式，`Timer.scheduledTimer` 默认只挂 default 模式，一下都不会响（Codex review 抓的）。视图被 LazyVStack 回收给别的行（`apply` 里 `dragPath` 变了）或拆掉（`dismantleNSView`）时 `reset()`：按旧身份撤高亮、停定时器。树里的高亮状态是 `DropTargetState`（记「哪一行 + 哪个目录」，离开只能撤自己那一行的，相邻文件行代表同一目录时 AppKit 会先报新行进入再报旧行离开）。右键与 ⌃点击在 `hitTest` 里放行，SwiftUI 的 `.contextMenu` 才会弹。别改回 `PressGesture` + `.onDrag`：两者争同一个 mouseDown，谁赢没有保证。拖放的判定在 `FileRename.validateMove` / `ProjectSession.moveProblem`，搬法与重命名共用 `move(_:to:verb:)` + `didRename`（跨目录时还会 `tree.reveal` 新位置并选中）。松手就搬不弹确认（访达也不问），`ProjectSession.notify` 在状态栏给一条带「撤销」的提示（`bannerAction`，8 秒自动消失；撤销就是再搬回去，也能再撤销）。撤销前核对搬过去那个文件的身份（设备号 + inode，`fileIdentity`），这几秒里它被删了 / 改名了 / 被别的文件顶了就只提示不搬。存在性判断一律 lstat 语义（`entryExists`），失效的符号链接也占着名字；自包含判断除字符串前缀外还按真实路径（目标目录可能是指进源目录的链接）再查一次。出错的提示仍直接赋 `banner`、不自动消失。离屏合成点击的回归测试在 `TreeInteractionTests`；拖拽会话本身要真实的事件循环，测不了。
