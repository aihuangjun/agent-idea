...
| `DesignSystem` | 深色主题常量、`ContentRenderer`（WKWebView + render.js 的桥）、通用控件、文件图标、随包分发的 `Resources/web` | Core |
| `Workbench` | `WorkbenchModel`（打开着的多个项目、阅读偏好、共用的 WebView）+ `ProjectSession`（单个项目：树、git、标签、提交草稿）与全部视图、窗口、菜单、更新器 | Core, DesignSystem |
...
- **正文渲染都在 WebView 里**（代码、Markdown、diff、图片）。Swift 只送 payload，契约写在 `render.js` 顶部注释，改一边必须改另一边；`DesignSystemTests` 会检查 render.js 暴露的入口名。
...
- **多项目共用一个 WebView**：`ProjectSession.renderActiveTab` 只在自己是当前项目时才画（`isActiveSession`），切项目由 `WorkbenchModel.activate` 负责重画。
...
窗口偏好（工具窗口宽度、diff 模式、缩放、自动换行）与上次开着的项目列表在 UserDefaults。
